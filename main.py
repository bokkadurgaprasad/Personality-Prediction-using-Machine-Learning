...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
  ...
  #  result.geometry('700x550')
    ...
    quitBtn = Button(result, text="Exit", command =lambda: exit_program(result)).pack()
...
"""
    ...
    result.protocol("WM_DELETE_WINDOW", lambda: exit_program(result))
...
def OpenFile(b4):
    ...
    name = filedialog.askopenfilename(initialdir="C:/Users/Batman/Documents/Programming/tkinter/",
                            filetypes =(('All files', '*'), ("Document","*.docx*"), ("PDF","*.pdf*")),
                           title = "Choose a file."
                           )
    try:
...
if __name__ == "__main__":
    ...
    def quit_main_loop(event):
        root.quit()
    # Bind the 'Escape' key to the quit_main_loop function
    root.bind('<Escape>', quit_main_loop)
    root.mainloop()